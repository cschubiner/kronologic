...
export function buildCNF(config) {
  ...
  if (config.scenarios.s7) {
    ...
    const pairVars = Array.from({ length: T }, () =>
      Array.from({ length: R.length }, () =>
        Array.from({ length: C.length }, () => Array(C.length).fill(null)),
      ),
    );

    const pairPresence = Array.from({ length: C.length }, () =>
      Array(T).fill(null),
    );
    ...
    for (let t = 0; t < T; t++) {
      for (let ri = 0; ri < R.length; ri++) {
        for (let ci = 0; ci < C.length; ci++) {
          for (let cj = ci + 1; cj < C.length; cj++) {
            const pairVar = vp.get(`Pair_${C[ci]}_${C[cj]}_${t}_${R[ri]}`);
            pairVars[t][ri][ci][cj] = pairVar;
            pairVars[t][ri][cj][ci] = pairVar;
            ...
            const reverse = [pairVar, -X(ci, t, ri), -X(cj, t, ri)];
            for (let ck = 0; ck < C.length; ck++) {
              if (ck === ci || ck === cj) continue;
    ...
    for (let ci = 0; ci < C.length; ci++) {
      for (let t = 0; t < T; t++) {
        const pairPresenceVar = vp.get(`PairPresence_${C[ci]}_${t}`);
        pairPresence[ci][t] = pairPresenceVar;

        const options = [];
        for (let ri = 0; ri < R.length; ri++) {
          for (let cj = 0; cj < C.length; cj++) {
            if (ci === cj) continue;
            const pv = pairVars[t][ri][ci][cj];
            if (pv) options.push(pv);
          }
        }

        if (options.length) {
          clauses.push([-pairPresenceVar, ...options]);
          for (const opt of options) {
            clauses.push([-opt, pairPresenceVar]);
          }
        } else {
          clauses.push([-pairPresenceVar]);
        }
      }
    }

    const pairTotals = pairPresence.map((vars, idx) =>
      buildTotalizer(vars, vp, clauses, `PairCount_${C[idx]}`),
    );

    for (let ci = 0; ci < C.length; ci++) {
      if (requiredKills > 0) {
        if (requiredKills - 1 < pairTotals[ci].length) {
          clauses.push([-AGG[ci], pairTotals[ci][requiredKills - 1]]);
        } else {
          clauses.push([-AGG[ci]]);
        }
      }
      ...
      for (let cj = 0; cj < C.length; cj++) {
        if (ci === cj) continue;
        for (let m = 1; m <= pairTotals[cj].length; m++) {
          const otherLit = pairTotals[cj][m - 1];
          if (!otherLit) continue;
          const aggIdx = 2 * m - 1;
          if (aggIdx < pairTotals[ci].length) {
            clauses.push([-AGG[ci], -otherLit, pairTotals[ci][aggIdx]]);
          } else {
            clauses.push([-AGG[ci], -otherLit]);
          }
        }
      }
    }
