...
export function buildCNF(config) {
  ...
  // S10: Contagion (alphabetically first room is contagious)
  let INF = null;
  if (config.scenarios && config.scenarios.s10) {
    if (!R.length) throw new Error("S10 requires at least one room");

    const contagiousRoom = [...R].sort()[0];
    const contagiousIdx = Ridx.get(contagiousRoom);
    INF = Array.from({ length: C.length }, () => Array(T).fill(null));
    const visitSeeds = [];

    for (let ci = 0; ci < C.length; ci++) {
      for (let t = 0; t < T; t++) {
        const infVar = vp.get(`S10INF_${C[ci]}_${t}`);
        INF[ci][t] = infVar;

        // Infection persists once acquired
        if (t > 0) {
          clauses.push([-INF[ci][t - 1], infVar]);
        } else {
          // Infection at t=1 can only come from the contagious room
          clauses.push([-infVar, X(ci, 0, contagiousIdx)]);
        }

        // Visiting the contagious room immediately infects
        clauses.push([-X(ci, t, contagiousIdx), infVar]);
        visitSeeds.push(X(ci, t, contagiousIdx));
      }
    }

    // Contact with an infected person at the previous timestep causes infection
    for (let ci = 0; ci < C.length; ci++) {
      for (let t = 1; t < T; t++) {
        const contactVars = [];
        for (let ri = 0; ri < R.length; ri++) {
          for (let cj = 0; cj < C.length; cj++) {
            if (ci === cj) continue;
            const contact = vp.get(
              `S10CONTACT_${C[ci]}_${t}_${R[ri]}_${C[cj]}`,
            );
            contactVars.push(contact);

            clauses.push([-contact, INF[cj][t - 1]]);
            clauses.push([-contact, X(ci, t, ri)]);
            clauses.push([-contact, X(cj, t, ri)]);
            clauses.push([
              -INF[cj][t - 1],
              -X(ci, t, ri),
              -X(cj, t, ri),
            ...
            clauses.push([-contact, INF[ci][t]]);
          }
        }

        // New infections must have a cause: prior infection, contagious room, or contact
        if (contactVars.length) {
          clauses.push([
            -INF[ci][t],
            INF[ci][t - 1],
            X(ci, t, contagiousIdx),
            ...contactVars,
          ]);
        } else {
          clauses.push([-INF[ci][t], INF[ci][t - 1], X(ci, t, contagiousIdx)]);
        }
      }
    }

    // Someone must visit the contagious room
    clauses.push(visitSeeds);
...
export function solveAndDecode(cfg) {
  ...
  if (cfg.scenarios && cfg.scenarios.s10) {
    ...
    for (const ch of C) infectionTimes[ch] = null;

    for (let t = 0; t < T; t++) {
      for (const ch of C) {
        if (schedule[ch][t] === contagiousRoom && infectionTimes[ch] === null) {
          infectionTimes[ch] = t + 1;
          infectedSet.add(ch);
        }
      }

      const infectedBefore = new Set();
      for (const ch of C) {
        if (infectionTimes[ch] !== null && infectionTimes[ch] <= t) {
          infectedBefore.add(ch);
        }
      }

      for (const room of R) {
        const occupants = C.filter((c) => schedule[c][t] === room);
        const hasInfected = occupants.some((c) => infectedBefore.has(c));
        if (hasInfected) {
          for (const occ of occupants) {
            if (infectionTimes[occ] === null) {
              infectionTimes[occ] = t + 1;
              infectedSet.add(occ);
            }
          }
        }
    ...
    for (const ch of C) {
      if (infectionTimes[ch] !== null) {
        infectionOrder.push({ character: ch, time: infectionTimes[ch] });
      }
    }
    infectionOrder.sort((a, b) =>
      a.time === b.time
    ...
    priv.contagion = {
      ...
      infected: Array.from(infectedSet),
    };
  }
