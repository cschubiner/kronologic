...
export function buildCNF(config) {
  ...
  if (config.scenarios.s7) {
    ...
    const pairAtTime = Array.from({ length: C.length }, () =>
      Array(T).fill(null),
    );
    const pairWitnesses = Array.from({ length: C.length }, () =>
      Array.from({ length: T }, () => []),
    );

    for (let ci = 0; ci < C.length; ci++) {
      for (let t = 0; t < T; t++) {
        pairAtTime[ci][t] = vp.get(`S7PairTime_${C[ci]}_${t}`);
      }
    }
    ...
    for (let ci = 0; ci < C.length; ci++) {
      for (let cj = ci + 1; cj < C.length; cj++) {
        for (let t = 0; t < T; t++) {
          for (let ri = 0; ri < R.length; ri++) {
            const detail = vp.get(
              `S7PairDetail_${C[ci]}_${C[cj]}_${t}_${R[ri]}`,
            );

            pairWitnesses[ci][t].push(detail);
            pairWitnesses[cj][t].push(detail);
            ...
            const requireDetail = [detail, -X(ci, t, ri), -X(cj, t, ri)];
            for (let ck = 0; ck < C.length; ck++) {
              if (ck === ci || ck === cj) continue;
              requireDetail.push(X(ck, t, ri));
            }
            clauses.push(requireDetail);

            clauses.push([-detail, pairAtTime[ci][t]]);
            clauses.push([-detail, pairAtTime[cj][t]]);
          }
        }
      }
    }
    ...
    for (let ci = 0; ci < C.length; ci++) {
      for (let t = 0; t < T; t++) {
        const witnesses = pairWitnesses[ci][t];
        if (witnesses.length === 0) {
          clauses.push([-pairAtTime[ci][t]]);
        } else {
          clauses.push([-pairAtTime[ci][t], ...witnesses]);
        }
      }
    }

    const pairTotals = pairAtTime.map((times, ci) =>
      buildTotalizer(times, vp, clauses, `S7PairTotal_${C[ci]}`),
    );
    ...
    for (let ci = 0; ci < C.length; ci++) {
      ...
      if (requiredKills > totals.length) {
        ...
        const threshold = totals[requiredKills - 1];
        clauses.push([-AGG[ci], threshold]);
      }
    }
    ...
    for (let ci = 0; ci < C.length; ci++) {
      ...
      for (let cj = 0; cj < C.length; cj++) {
        ...
        for (let m = 1; m <= otherTotals.length; m++) {
          const otherAtLeast = otherTotals[m - 1];
          if (!otherAtLeast) continue;
          const requiredCount = 2 * m;
          if (requiredCount > aggTotals.length) {
            clauses.push([-AGG[ci], -otherAtLeast]);
          } else {
            const aggAtLeast = aggTotals[requiredCount - 1];
            clauses.push([-AGG[ci], -otherAtLeast, aggAtLeast]);
          }
        }
      }
    }
  ...
  if (config.scenarios && config.scenarios.s8) {
    ...
    for (let ci = 0; ci < C.length; ci++) {
      if (freezeKillsBeforeFinal[ci].length === 0) {
        throw new Error(
          "S8 requires a kill opportunity before the final timestep",
        );
      }
