...
describe('S7: Aggrosassin Scenario', () => {
  ...
  it('should ensure aggrosassin has at least twice the 1-on-1 meetings as anyone else', () => {
    const cfg = {
      rooms: ['A', 'B', 'C', 'D'],
      edges: [['A', 'B'], ['B', 'C'], ['C', 'D']],
      chars: ['P', 'Q', 'R', 'S', 'T'],
    ...
    testWithThreshold(cfg, (res, cfg) => {
        ...
        for (const room of cfg.rooms) {
          ...
          if (charsInRoom.length === 2) {
            for (const char of charsInRoom) {
              pairCounts.set(char, pairCounts.get(char) + 1)
            }
          }
        }
      }
      ...
      const maxOther = Math.max(...cfg.chars.filter(c => c !== agg).map(c => pairCounts.get(c)))
