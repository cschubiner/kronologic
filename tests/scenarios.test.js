...
describe("S6: Phantom + Lovers Scenario (S2 + S5)", () => {
  ...
  it.skip("should ensure both lovers meet all non-phantom non-lovers", () => {
    for (let seed = 1530; seed < 1535; seed++) {
      ...
      const nonPhantomNonLovers = cfg.chars.filter(
        (c) => c !== phantom && c !== lover1 && c !== lover2,
      );
...
describe("S7: Aggrosassin Scenario", () => {
  ...
  it("should have aggrosassin in at least twice as many 1-on-1 meetings as anyone else", () => {
    const cfg = {
      rooms: ["A", "B", "C", "D"],
      edges: [
        ["A", "B"],
    ...
    testWithThreshold(cfg, (res, cfg) => {
      ...
      // Count 1-on-1 meetings per character
      const pairCounts = new Map(cfg.chars.map((c) => [c, 0]));
      for (let t = 0; t < cfg.T; t++) {
        for (const room of cfg.rooms) {
          const charsInRoom = cfg.chars.filter(
            (c) => res.schedule[c][t] === room,
          ...
          if (charsInRoom.length === 2) {
            pairCounts.set(charsInRoom[0], pairCounts.get(charsInRoom[0]) + 1);
            pairCounts.set(charsInRoom[1], pairCounts.get(charsInRoom[1]) + 1);
          }
        }
      }

      const aggAloneCount = pairCounts.get(agg) || 0;
      const others = cfg.chars
        .filter((c) => c !== agg)
        .map((c) => pairCounts.get(c) || 0);
      const nextHighest = others.length ? Math.max(...others) : 0;

      expect(aggAloneCount).toBeGreaterThanOrEqual(nextHighest * 2);
    });
  });
...
describe("S6 Verification Tests", () => {
  it.skip("should verify phantom is separate from lovers", () => {
    ...
    for (let seed = 999; seed < 1009; seed++) {
      ...
      for (let t = 0; t < cfg.T; t++) {
        const phantomRoom = res.schedule[phantom][t];
        const othersInRoom = cfg.chars.filter(
          (c) => c !== phantom && res.schedule[c][t] === phantomRoom,
        );
