...
describe('S7: Aggrosassin Scenario', () => {
  ...
  it('should have aggrosassin alone with people far more often than anyone else', () => {
    const cfg = {
      rooms: ['A', 'B', 'C', 'D'],
      edges: [['A', 'B'], ['B', 'C'], ['C', 'D']],
      chars: ['P', 'Q', 'R', 'S', 'T'],
    ...
    testWithThreshold(cfg, (res, cfg) => {
      ...
      // Count one-on-one meetings for each character
      const soloCounts = new Map()
      for (const char of cfg.chars) soloCounts.set(char, 0)

      for (let t = 0; t < cfg.T; t++) {
        for (const room of cfg.rooms) {
          const charsInRoom = cfg.chars.filter(c => res.schedule[c][t] === room)
          if (charsInRoom.length === 2) {
            for (const char of charsInRoom) {
              soloCounts.set(char, soloCounts.get(char) + 1)
            }
          }
        }
      }

      const aggAloneCount = soloCounts.get(agg)
      const otherCounts = cfg.chars
        .filter(char => char !== agg)
        .map(char => soloCounts.get(char))
      const maxOtherCount = otherCounts.length ? Math.max(...otherCounts) : 0

      // Aggrosassin should have at least twice as many 1-on-1 meetings as anyone else
      expect(aggAloneCount).toBeGreaterThanOrEqual(maxOtherCount * 2)
    })
  })
  ...
  it('should work with mustMove constraint', () => {
    ...
    testWithThreshold(cfg, (res, cfg) => {
      ...
      const maxOtherCount = Math.max(
        0,
        ...cfg.chars.filter(char => char !== agg).map(char => soloCounts.get(char))
      )
