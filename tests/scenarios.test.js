...
describe("S10: Contagion Scenario", () => {
  it("infects first visitors to the earliest room and records the order", () => {
    const cfg = {
      rooms: ["Study", "Atrium", "Balcony"],
      edges: [
        ["Atrium", "Study"],
        ["Atrium", "Balcony"],
      ],
      chars: ["Ada", "Ben", "Cora", "Dia"],
      T: 5,
      mustMove: false,
      allowStay: true,
      scenarios: { s10: true },
    ...
    testWithThreshold(cfg, (res, cfg) => {
      ...
      expect(contagion).toBeTruthy();

      const contagiousRoom = [...cfg.rooms].sort()[0];
      expect(contagion.contagious_room).toBe(contagiousRoom);
      expect(contagion.infection_order.length).toBeGreaterThan(0);

      const earliest = Math.min(
        ...contagion.infection_order.map((e) => e.time),
      );
      const firstWave = contagion.infection_order.filter(
        (e) => e.time === earliest,
      );
      for (const infection of firstWave) {
        expect(res.schedule[infection.character][earliest - 1]).toBe(
          contagiousRoom,
        );
      }
    });
  ...
  it("spreads infection from prior carriers or the contagious room", () => {
    const cfg = {
      rooms: ["Library", "Observatory", "Vault"],
      edges: [
        ["Library", "Observatory"],
        ["Observatory", "Vault"],
      ],
      chars: ["Liam", "Mona", "Noah", "Opal", "Pia"],
      T: 6,
      mustMove: false,
      allowStay: true,
      scenarios: { s10: true },
    ...
    testWithThreshold(cfg, (res, cfg) => {
      ...
      const contagiousRoom = contagion.contagious_room;

      for (const [character, time] of Object.entries(infectionTimes)) {
        if (time === null) continue;
        const timestep = time - 1;
        const room = res.schedule[character][timestep];
        const infectedBefore = new Set(
          Object.entries(infectionTimes)
            .filter(([, t]) => t !== null && t < time)
            .map(([ch]) => ch),
        );

        const roommates = cfg.chars.filter(
          (other) =>
            other !== character && res.schedule[other][timestep] === room,
        );
        const metCarrier = roommates.some((other) => infectedBefore.has(other));

        expect(room === contagiousRoom || metCarrier).toBe(true);
      }

      for (let i = 1; i < contagion.infection_order.length; i++) {
        expect(contagion.infection_order[i].time).toBeGreaterThanOrEqual(
          contagion.infection_order[i - 1].time,
        );
      }
    });
  });
