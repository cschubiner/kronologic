...
describe("S7: Aggrosassin Scenario", () => {
  ...
  it("should make the aggrosassin dominate one-on-one meetings", () => {
    const cfg = {
      rooms: ["A", "B", "C", "D"],
      edges: [
        ["A", "B"],
    ...
    testWithThreshold(cfg, (res, cfg) => {
      ...
      const meetingCounts = Object.fromEntries(cfg.chars.map((c) => [c, 0]));
        ...
        for (const room of cfg.rooms) {
          ...
          if (charsInRoom.length === 2) {
            for (const char of charsInRoom) {
              meetingCounts[char] = (meetingCounts[char] || 0) + 1;
            }
          }
        }
      }

      const aggMeetingCount = meetingCounts[agg] || 0;
      const minKills = Math.ceil(cfg.T / 2);
      expect(aggMeetingCount).toBeGreaterThanOrEqual(minKills);

      const maxOtherMeeting = Math.max(
        ...cfg.chars.filter((c) => c !== agg).map((c) => meetingCounts[c] || 0),
      );

      expect(aggMeetingCount).toBeGreaterThanOrEqual(maxOtherMeeting * 2);
    });
  });
